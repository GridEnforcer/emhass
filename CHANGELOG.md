...
- `treat_runtimeparams` now clamps each `soc_init` / `soc_final` **list** element to ITS OWN battery's `battery_minimum_state_of_charge_list[b]` / `battery_maximum_state_of_charge_list[b]`, not the scalar `battery_minimum_state_of_charge` (which only carries battery 0). Field 2026-05-28 (gridenforcer): a 2-battery install with house battery min_soc 0.25 (battery 0) and a V2G EV at 18 % SoC (battery 1, min relaxed to 0.18) saw the EV's `soc_init=0.18` clamped UP to 0.25 — so the EV's planned SOC trajectory (`SOC_opt1`) started at 25 % instead of the real 18 %, and the dashboard EV curve was misaligned with reality. The scalar-fallback path (single-battery, scalar soc_init) is unchanged. 2 new tests in `test_utils.py`. See gridenforcer_core-nrs follow-up.
- WebSocket statistics fetch (`RetrieveHass.get_data_websocket`) now picks the period adaptively from the requested window: `period="5minute"` for windows ≤10 days (high resolution within HA's short-term statistics retention), `period="hour"` for windows >10 days (long-term statistics, retained for years). Without this, ML training over `historic_days_to_retrieve > 10` returned mostly empty data because HA only retains 5-minute statistics for ~10 days regardless of `recorder.purge_keep_days`. The `recorder/statistics_during_period` WebSocket command keeps hourly aggregates for ~10 years on a default install, so this unlocks long ML training windows without bloating the recorder DB. Plus `treat_runtimeparams` now honors `use_websocket` from runtimeparams (merged into `retrieve_hass_conf`), so callers can opt into the WebSocket+statistics path per-call without editing the YAML config — useful for selectively routing only the ML-training fetches through LTSS while other actions stay on the default path. 2 new tests cover the period-adapts-to-window behavior and the runtimeparam passthrough. See gridenforcer_core-9nz.
- Route per-battery `_list` runtimeparams into `plant_conf` / `optim_conf`: add `associations.csv` entries for `battery_is_dc_coupled_list`, `battery_charge_efficiency_list`, `battery_discharge_efficiency_list`, `battery_{minimum,maximum,target}_state_of_charge_list`, `battery_{charge,discharge}_power_max_list`, `battery_nominal_energy_capacity_list`, `weight_battery_{charge,discharge}_list`, and `set_no{charge_from,discharge_to}_grid_list`. Without these, `treat_runtimeparams` silently dropped every `_list` key and the solver fell back to scalar broadcasts — most visibly, `battery_is_dc_coupled_list=[False]` for an AC-coupled EV charger was ignored, so EMHASS routed the Tesla/V2G discharge through the hybrid inverter's DC bus instead of directly to the AC load.
