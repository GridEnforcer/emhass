...
## GridEnforcer fork — 2026-09-05 (ge-mo3z)

- **`battery_salvage_price` now actually reaches the LP over the REST API.** The ge-zues runtime param was registered in `BATT_ARRAY_PARAMS_OPTIM_CONF` and the `OptimizationCache` key but never got an `associations.csv` row, and `treat_runtimeparams` routes runtime keys into `optim_conf` only through that file — so every naive-mpc call silently dropped it, `_battery_salvage_mask()` was all-False and the terminal `soc_final` pin was built exactly as upstream. Live since addon 0.18.1-ge14: plans charged "just enough to drain to the target" instead of filling into cheap PV (Utegard 2026-09-05: 17 kW surplus for 7.5 h, battery stopped at 85.5 % and idled). The fork's own tests passed because they inject via `optim_conf` directly. Added the association row, a `config_defaults.json` default (0.0), `param_definitions.json` entries for all three fork battery params (the schema-contract test had been red on the fork since the first extension), and `tests/test_salvage_value.py::TestSalvagePriceRuntimeRouting` which drives `treat_runtimeparams` end to end for the scalar, per-battery-list, absent and sibling-param cases. Closes ge-mo3z.
